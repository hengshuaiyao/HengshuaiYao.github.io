...
In this paper, I discussed (1) why negative step-sizes make sense; (2) the diagonal-matrix step-size has the same projection power as a full-sized matrix; (3) the gradient descent update is data; and (4) we can use the gradient descent update data to learn and plan step-sizes for an acceleration of gradient descent. 
