...
Unfortunately, these three works were not independent. We had one phone conversation (for a Ph.D candidate interview in March 2008) and lots of email discussions before their NIPS 2008 submssion. I did experiments for GTD and wrote a two-page PDF to point out (1) two time-scale formulation of GTD is slow; (2) the two step-sizes are not easy to use. Someone also asked me to send the Boyan chain example code to his student. Later on, the TDC paper's Boyan chain experiment was based on my code. In my experiment, I used on-policy learning to study the convergence rate of off-policy learning algorithms. This was the first time in literature to use this experimentation method (e.g., read the importance sampling off-policy TD papers By Sutton, et. al. in 2000 and 2001 to see their experimentation method). My experimentation method was adopted in all the experiments of the TDC paper. Later research (e.g., see [TDRC](https://arxiv.org/pdf/2007.00611.pdf)) uses this method as a standard way to study off-policy learning algorithms. 
