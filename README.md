...
In this paper, I discussed (1) a data perspective for gradient descent, and the possibility of using it to learn and plan step-sizes for an acceleration of gradient descent. (2) a new understanding of individual step-sizes for parameters, that the collection of them, in the form of diagonal matrix as we explored in the paper, can be viewed as a multi-step transition model for gradient descent update. (4) why negative step-sizes make sense for both deterministic and stochastic settings for fast convergence. 
