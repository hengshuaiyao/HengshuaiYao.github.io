...
To prospective students: Sorry, I couldn't take any student because I'm just an adjunct prof at the department. I could co-supervise with someone in the department. However, there are too many things I need to figure out (research and the meaning of life), which may take an infinite time. I also want to be focused on my work at Sony AI.     
...
However, my contributions were not recognized or credited by the literature for some reasons that should have happened in the first place. The GTD and GTD2/TDC papers received about 1000+ citations at the time of writing this. My paper received 13 by February 2023. 

Here is this [preconditioning TD paper](papers/ptd.pdf) (ICML, 2008) where GTD (NIPS, 2008) was derived from. [Comparison](gtd_novelty.md).
We also had one phone conversation (for a Ph.D candidate interview in March 2008) and lots of email discussions before their NIPS 2008 submssion. I did experiments for GTD and wrote a two-page PDF to point out (1) two time-scale formulation of GTD is slow; (2) the two step-sizes are not easy to use. Someone also asked me to send the Boyan chain example code to his student. Later on, the TDC paper's Boyan chain experiment was based on my code. In my experiment, I used on-policy learning to study the convergence rate of off-policy learning algorithms. This was the first time in literature to use this experimention method (e.g., read the the importance sampling off-policy TD papers By Sutton, et. al. in 2000 and 2001 to see their experimentation method). My experimentation method was adopted in all the experiments of the TDC paper. Later research (e.g., see [TDRC](https://arxiv.org/pdf/2007.00611.pdf)) uses this method as a standard way to study off-policy learning algorithms.  

I had wished and waited patiently for the authors of the GTD, TDC/GTD2 papers to have a reflection (e.g., search  the email boxes?) on, **(1) whether the GTD and TDC papers took great advantage of the preconditioning paper without giving any credit; (2) whether my involvement at the time deserved the authorship**. 
...
Horde: I proposed learning many policies from a single stream of data and studied it in an offline framework called "[one collection for all](papers/one_collection_for_all.pdf)". Horde was proposed the same year with the main author knowing my work. My "one collection for all" paper was written and known to some Horde author from beginning because I was planning to go for this workshop at the time.     

ETD: Equation 13 of the [ETD paper](https://arxiv.org/pdf/1503.04269.pdf) (2015) is the key that drives the whole paper. It is just the PageRank formulation. Later some ETD authors started using some vector called "user interest" vector. It is actually the personalized PageRank. It was a commonplace form in the PageRank literature, but it was treated so new in RL because the ETD paper did not mention anything about this background. My [reinforcement ranking paper](papers/rr.pdf) (2013) is the first paper that points out the connection between reinforcement learning and the PageRank literature. Someone was in my defense committee when I did my candidacy based on this reverse Bellman equation, and he abondoned me (an internaltional student at the time) as my Ph.D supervisor because I wanted to purse this direction in 2011. Unfortunately, the person, who I highly respected for so many years, just said "No" when my new supervisor arranged my defense in 2014 and asked him whether he wanted to be part of it.
It's Okay in a normal situation. People have different interests and schedules. I was so heartbroken to hear that. I knew well what he did to me for all these. We worked together for three years in my Ph.D time. 

People in the planning literature started using "backward model" (heard from Yangchen) a few years ago; this is just based on the PageRank model. 
